...
const BlogSocialShare: React.FC<BlogSocialShareProps> = ({ post }) => {
  ...
  const generateShareData = (): ShareData => {
    const title = `${post.title} - Travel Story on Fojourn`;
    
    const description = post.excerpt || 
      `Read this amazing travel story by ${post.author_display_name}. Discover new destinations and travel inspiration on Fojourn.`;
    
    const url = window.location.href;
    
    // Use hero image if available
    const imageUrl = post.hero_image_url || undefined;
  ...
  const handleFacebookShare = () => {
    try {
      ...
      console.log('🔥 Facebook Blog Share Debug:');
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        ...
        setSnackbarMessage('Blog post shared successfully!');
        setSnackbarOpen(true);
      } catch (error) {
