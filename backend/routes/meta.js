...
// NEW ROUTE: Alternative sharing URL to bypass Facebook cache
router.get('/share/:slug', async (req, res) => {
  try {
    const { slug } = req.params;

    // Get memory with user info and media directly (same logic as share endpoint)
    const [memories] = await pool.execute(`
      SELECT 
        te.*,
        u.username,
    ...
    // Determine image URL with even more aggressive cache busting
    let imageUrl = `${baseUrl}/fojourn-icon.png`; // Default fallback (correct filename)
    if (media.length > 0) {
      // Use current timestamp + random number for maximum cache busting
      const timestamp = Date.now();
...
<head>
    ...
    <!-- Required Open Graph properties with aggressive cache busting -->
    <meta property="og:url" content="${baseUrl}/api/meta/share/${slug}" />
    <meta property="og:type" content="article" />
    <meta property="og:title" content="${title}" />
    <meta property="og:description" content="${description}" />
    <meta property="og:image" content="${imageUrl}" />
    ...
    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:url" content="${baseUrl}/api/meta/share/${slug}" />
    <meta name="twitter:title" content="${title}" />
    <meta name="twitter:description" content="${description}" />
    <meta name="twitter:image" content="${imageUrl}" />
...
<body>
    ...
    <script>
        ...
        if (isFacebookBot) {
            document.body.innerHTML += '<p>Facebook sharing page with fresh meta tags.</p>';
        } else if (isOtherBot) {
            document.body.innerHTML += '<p>Social media sharing page.</p>';
        }
    </script>
</body>
</html>`;
